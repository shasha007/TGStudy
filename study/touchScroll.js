...
TouchScroll.prototype={
	...
	slide:function(flag,distance,during,fx,callback){
		var _this=this, _callback=callback||new Function(),
			_init=parseInt(this.css(this.element,this.property[flag][0])),
			_change=distance,
			_stime=0,
			_during=during||400,
			_fx=fx||'ease-out';
		function animate(t,b,c,d,fx){
			var re;
			switch(fx){
				case 'ease-in-out':
	...
	start:function(e){
		clearTimeout(this.timer[0]); clearTimeout(this.timer[1]);
	...
	move:function(e){
		...
		if(this.target){
			var barLength=parseInt(this.css(this[this.property[this.flag][6]],this.property[this.flag][2]));
